...
class LiveDataTestObserver<T> private constructor() : Observer<T> {
    ...
    override fun onChanged(value: T) {
        ...
        for (consumer: Consumer<T> in onChanged) {
    ...
    fun assertValue(expected: T): LiveDataTestObserver<T> {
        ...
        if (notEquals(value, expected as Any)) {
            throw fail(
                "Expected: " + valueAndClass(expected) + ", Actual: " + valueAndClass(
                    value
                )
            )
        }
    ...
    fun assertValueHistory(vararg values: T): LiveDataTestObserver<T> {
        ...
        for (valueIndex in 0 until size) {
            val historyItem = valueHistory[valueIndex] as Any
            val expectedItem = values[valueIndex] as Any
    ...
    fun assertValueHistory(valuePredicate: Function<T, Boolean>): LiveDataTestObserver<T> {
        val size = valueHistory.size
        for (valueIndex in 0 until size) {
            val value: T = valueHistory[valueIndex]
            if (valuePredicate.apply(value)) {
        ...
        throw fail(
            (
                "Value not matches predicate " +
                    valuePredicate.toString() + ", which was expected."
                )
        )
    }
    ...
    companion object {
        ...
        fun <T> test(liveData: LiveData<T>): LiveDataTestObserver<T> {
            val observer: LiveDataTestObserver<T> = LiveDataTestObserver()
