...
class GetOtherProfileUseCaseImplTest {
    private lateinit var getOtherProfileUseCase: GetOtherProfileUseCase
    private val profileRepository: ProfileRepository = mock()
    private val localizationRepository: LocalizationRepository = mock()

    @BeforeEach
    fun setUp() {
        getOtherProfileUseCase = GetOtherProfileUseCaseImpl(
            profileRepository,
    ...
    fun `test execute get profile by username success`() = runTest {
        ...
        whenever(profileRepository.getOtherProfile(userNameMock, fieldMock, "th"))
            .thenReturn(flowOf(response))

        getOtherProfileUseCase.execute(userNameMock, fieldMock).collect { result ->
