...
private var _appliation: Application? = null
    private set(value) {
        field = value
        if (value != null) {
            BaseApplicationProvider.applicationListeners.forEach {
                it.invoke(value)
            }
        }
    }
...
class AppContextProvider : EmptyProvider() {
    override fun onCreate(): Boolean {
        ...
        // Comment this line for prevent crash when run instrumented test
        FirebaseInstallations.getInstance().id
            .addOnCompleteListenerWithNewExecutor { task ->
                if (task.isSuccessful) {
                    Timber.i("[Firebase] Installations: " + "Installation ID: " + task.getResult())
                } else {
