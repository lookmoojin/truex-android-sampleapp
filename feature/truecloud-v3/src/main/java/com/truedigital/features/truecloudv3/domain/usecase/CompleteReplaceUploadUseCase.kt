...
interface CompleteReplaceUploadUseCase {
    fun execute(id: Int): Flow<TrueCloudV3Model>
}

class CompleteReplaceUploadUseCaseImpl @Inject constructor(
    ...
    override fun execute(id: Int): Flow<TrueCloudV3Model> {
        return flow {
            ...
            completeUploadRepository.callReplaceComplete(objectId, taskUploadModel?.coverImageSize)
        }.map { response ->
            response.data?.convertToTrueCloudV3Model()
                ?: error(TrueCloudV3ErrorMessage.ERROR_COMPLETE_UPLOAD)
        }
    }
}
