...
class SearchWithCategoryUseCaseTest {
    private lateinit var searchWithCategoryUseCase: SearchWithCategoryUseCase
    ...
    fun `test get search result`() = runTest {
        ...
        coEvery {
            searchFileRepository.searchWithCategory(any(), any(), any(), any(), any())
        } returns flowOf(expectedResult)

        val flow = searchWithCategoryUseCase.execute(
